...
@Composable
fun MockUp(modifier: Modifier = Modifier.Companion) {
    Column(modifier = modifier.fillMaxSize().padding(LogApp.BTN_PADDING)) {
        Row(modifier = Modifier.fillMaxHeight(LogApp.LOG_MAX_HEIGHT_FRACTION)) {
            LazyColumn(
                modifier = Modifier.background(LogApp.LOG_LIST_BGCOLOR).fillMaxSize(),
        ...
        Row(modifier = Modifier.fillMaxWidth().weight(LogApp.CTRLS_MAX_HEIGHT_FRACTION)
        ) {
